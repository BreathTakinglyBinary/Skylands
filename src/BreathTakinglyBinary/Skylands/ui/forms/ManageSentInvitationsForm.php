...
class ManageSentInvitationsForm extends SimpleForm{

    /** @var IsleHelperInvite[] */
    protected $invites;

    public function __construct(Player $isleOwner, ?Form $previousForm = null){
        parent::__construct(TranslationManager::getTranslatedMessage("FORM_TITLE_MANAGE_SENT_INVITES"), $previousForm);
        $this->invites = IsleManager::getInstance()->getInviteManager()->getInvitationsByIsleOwner($isleOwner);
        if(count($this->invites) < 1){
            $this->setContent(TranslationManager::getTranslatedMessage("FORM_MESSAGE_NO_INVITES_RECEIVED"));
        }
        foreach($this->invites as $inviteeName => $invite){
            if(!$invite->isExpired()){
                $this->addButton($inviteeName, $inviteeName);
            }
        }
    ...
    public function onResponse(Player $player, $data) : void{
        ...
        $player->sendForm(new HandleSentInvitationForm($this->invites[$data]));
    }
}